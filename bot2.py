        ...
        for idx, resp in enumerate(collected):
            ...
            if btn:
                ...
                try:
                    ...
                    # One bounded retry with a fresh copy before giving up.
                    if code == 50035 and idx == 0:
                        try:
                            fresh = await channel.fetch_message(resp.id)
                            retry_btn = None
                            for row in getattr(fresh, "components", None) or []:
                            ...
                            if retry_btn is not None:
                                ...
                                clicked_ok = True
                                logger.info(
                                    "[%s] Recovered from COMPONENT_VALIDATION_FAILED via message refetch + retry.",
                                    cmd_name,
                                )
                        except Exception as retry_exc:
                            logger.warning(
                                "[%s] Retry after COMPONENT_VALIDATION_FAILED also failed: %s",
                                cmd_name, retry_exc,
                            )
                    if not clicked_ok:
        ...
        try:
            ...
            while True:
                ...
                try:
                    ...
                    # the live event was missed. Re-check cheaply while waiting.
                    await recover_recent()
                    continue
                return msg
        finally:
