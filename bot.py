    ...
    async def safe_execute(obj, func, *args, **kwargs):
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                # "Did not receive a response from Discord" is raised by
                # discord.py-self when *invoking* a slash command: it sends
                # the interaction, then waits for a gateway event confirming
                # Discord processed it, and that confirmation never arrives
                # in time. This can be plain backend/network latency, but
                # it's also the exact symptom of invoking a stale cached
                # command object -- if the target bot re-registers or
                # updates its application commands, Discord can silently
                # drop an interaction built from the old command reference
                # instead of returning a clear error. This used to fall
                # into the generic `except Exception` branch below, which
                # gives up immediately with no retry -- so once the cache
                # went stale, every future call to that command would fail
                # forever until something else happened to refresh it.
                # Fix: refresh the command cache and retry with a freshly
                # resolved command object, bounded by MAX_RETRIES.
                if attempt < MAX_RETRIES - 1:
                    channel_arg = args[0] if args else None
                    cmd_key = getattr(func, "qualified_name", None) or getattr(func, "name", None)
                    logger.warning(
                        f"safe_execute[{func_name}]: {e} — refreshing command cache and retrying "
                        f"(attempt {attempt + 1}/{MAX_RETRIES})."
                    )
                    if cmd_key and channel_arg is not None and hasattr(channel_arg, "application_commands"):
                        try:
                            await obj.get_commands(channel_arg, force_refresh=True)
                            refreshed = obj.resolve_dynamic_command(cmd_key)
                            if refreshed is not None:
                                func = refreshed
                        except Exception as refresh_exc:
                            logger.warning(f"safe_execute[{func_name}]: command refresh during retry failed: {refresh_exc}")
                    await asyncio.sleep(1.5 + random.uniform(0, 1.0))
                    continue
                logger.error(f"safe_execute[{func_name}] gave up after {MAX_RETRIES} attempts: {e}")
                obj.config["last_error"] = str(e)
                await obj.send_alert(f"⚠️ {func_name} raised InvalidData", str(e), severe=False)
                return None
            except Exception as e:
...
if __name__ == "__main__":
    # BUGFIX: this used to just call logger.error() on missing/invalid env
    # vars and then fall through with no sys.exit() -- the process reached
    # end-of-script and exited (code 0) almost immediately after. Worse,
    # logger.error() only *enqueues* onto the background QueueListener
    # thread set up above; that thread is a daemon and gets killed the
    # instant the main thread exits, so there was a real race where the
    # process could exit before the listener thread ever got scheduled to
    # actually write the message out. On Railway this showed up as
    # literally nothing past "Starting Container" in the logs -- the one
    # line that would have explained why was exactly the line most likely
    # to get lost.
    missing = []
    if not TOKEN:
    ...
    if not TARGET_BOT_ID:
        missing.append("TARGET_BOT_ID")

    if missing:
        # Print directly to stderr and flush immediately -- do not rely on
        # the queued logger alone for the one message that explains a fatal
        # startup failure to whoever is reading the deploy logs.
        print(
            f"[FATAL] Missing/invalid required environment variable(s): {', '.join(missing)}. "
            f"Set them in Railway's Variables tab and redeploy.",
            file=sys.stderr,
            flush=True,
        )
        logger.error("Environment file missing critical deployment requirements: %s", ", ".join(missing))
        _queue_listener.stop()
        sys.exit(1)

    # Unconditional, unmissable boot marker -- if this line never shows up
    # in the deploy logs, the problem is before this point (e.g. a bad
    # import or a requirements.txt issue), not in the bot's own startup
    # logic below.
    print("[BOOT] bot.py starting up...", flush=True)
    ...
    try:
        ...
        _STOP_EVENT.set()
        # Make sure any buffered log records are actually flushed to
        # console/file before the process exits, instead of risking them
        # being dropped along with the daemon listener thread.
        _queue_listener.stop()
        print("\nCIPHER stopped.", flush=True)
